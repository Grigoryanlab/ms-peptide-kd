...
%regress
function analyzeMS_sum2to9_ep2_alpha(outBase, dataFileXLS, aggregationType)
...
%         xcorr = allResult(j).xcor(i,idx);
        kds = allResult(j).Kd(i);
        kd_err = allResult(j).KdError(i);
        logkd_err = allResult(j).logKdError(i);
        kd_ints = allResult(j).KdHi(i) - allResult(j).KdLo(i);
        f = allResult(j).a(i);
        fE = allResult(j).aE(i);% alphaStd
...
%        if all(~isempty(kds)) && all(~isnan(kds))
        if all(~isempty(kds)) && all(~isnan(kds))&& all(f < 1 + fE)
...
for i = 1:length(keySeqs)
    ...
    fprintf(fid, '%s,%f,%f,%f,%f,%s,%f\n', keySeqs{i}, vals(i), err_obs, err_est, length(A(keySeqs{i})), num2str(alphaValue{i}), xVal(i));
end
fclose(fid);
...
aE(valid(okPoints)) = alphaStd(okPoints);
xcor(valid(okPoints)) = xc(okPoints);
KdLo(valid(okPoints)) = KdRange(:, 1);
KdHi(valid(okPoints)) = KdRange(:, 2);
result = struct('Kd', Kd, 'KdError', KdError, 'logKdError', logKdError, 'KdLo', KdLo, 'KdHi', KdHi, 'seqs', {procSeqs}, "a", a,"aE",aE, "xcor", xcor);
