...
%regress
function analyzeMS_sum3_a_dCnX(outBase, dataFileXLS)
...
fprintf(fid, 'sequence, Kd estimate, Kd standard error (obs), error (est), number of samples, alpha, dcorr, dcorr error,xcorr\n');
keySeqs = keys(M);
vals = values(M);
vals = [vals{:}];
alphaValue = values(G);
...
result = struct('Kd', Kd, 'KdLo', KdLo, 'KdHi', KdHi, 'seqs', {procSeqs},"a", a,"dcor",dcor,"xcor",xcor);
